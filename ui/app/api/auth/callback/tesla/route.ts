...
export async function GET(req: Request) {
  ...
  if (error) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?error=${encodeURIComponent(error)}`
    );
  }

  if (!code || !state) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?error=missing_code`
    );
  }
  ...
  if (!session) {
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?error=invalid_state`
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.redirect(
        `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?error=${encodeURIComponent(err)}`
      );
    }
    ...
    return NextResponse.redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?error=${encodeURIComponent(String(err))}`
    );
  }
  ...
  return NextResponse.redirect(
    `${process.env.NEXT_PUBLIC_BASE_URL ?? "http://localhost:3000"}/providers/tesla/connect?success=1`
  );
}
